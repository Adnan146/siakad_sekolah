						...
						<?php 
							...
							<div class="card">
								<div class="card-body">
									...
									<input type="date" name="tgl" class="form-control" value="<?=date('Y-m-d') ?>" style="background-color: #1648D8 ;color: #FFFFFF;">
									...
									<a href="?page=absen&act=update_absen&pelajaran=<?=$_GET['pelajaran']; ?>&id=<?= $s['id_mkelas'] ?>&mpl=<?= $m['id_mapel'] ?>" class="btn btn-warning"><i class="fas fa-edit"></i> Update Absesnsi</a>
									...
									if (isset($_POST['absen'])) {
										...
										for ($i =0; $i <=$total ; $i++) {
											...
											if ($cekAbsesnHariIni > 0) {
													...
													window.location.replace('?page=absen');
													} ,3000);   
													</script>";
